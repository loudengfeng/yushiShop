		...
		<swiper :current="tabCurrentIndex" class="swiper-box" duration="300" @change="changeTab">
			<swiper-item class="tab-content" v-for="(tabItem, tabIndex) in navList" :key="tabIndex">
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<view v-for="(item, index) in tabItem.orderList" :key="index" class="order-item">
						<view class="i-top b-b">
							...
							<text class="state" :style="{ color: item.stateTipColor }" v-if="item.OrderStatus == 1">待付款</text>
							<text class="state" :style="{ color: item.stateTipColor }" v-if="item.OrderStatus == 2">待发货</text>
                    ...
                    v-if="item.OrderStatus == 5">待评价</text> -->
						...
						<view class="goods-box" v-for="(itemGood, indexGood) in item.LineItemsList" :key="indexGood" style="margin-bottom: 20rpx;"
						 @click="jumpDetail(itemGood, item.OrderStatus)">
							<view class="goods-item">
								<image class="goods-img" :src="itemGood.ThumbnailsUrl" mode="aspectFill"></image>
							</view>
							<view class="right">
                ...
                ">
							...
							<button class="action-btn recom" @click.stop="pay(item.PayVoucherNumber,item.PaymentTypeId)">
								立即支付
							</button>
						</view>
