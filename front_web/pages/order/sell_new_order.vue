				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<view v-for="(item, index) in tabItem.orderList" :key="index" class="order-item">
						<view class="i-top b-b">
							...
							<text class="state" :style="{ color: item.stateTipColor }" v-if="item.orderStatus == 110">委卖订单</text>
							<text class="state" :style="{ color: item.stateTipColor }" v-if="item.orderStatus == 120">待收款</text>
							<text class="state" :style="{ color: item.stateTipColor }" v-if="item.orderStatus == 130">已收款</text>
		...
		methods: {
			...
			loadData(source) {
				...
				setTimeout(() => {
					...
					//买入订单
					 if (index == 0) {
						this.req_status(navItem.start, 110, index);
					}
					// 待付款
					else if (index == 1) {
						this.req_status(navItem.start,120, index);
					}	else if (index == 2) {
						this.req_status(navItem.start,130, index);
					}
					//已付款
					// else if (index == 2) {
					// 	this.req_status(navItem.start, 100, index);
