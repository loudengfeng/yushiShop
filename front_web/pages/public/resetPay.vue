<template>
	<view class="find_psw">
		<form @submit="loginNow" class="form1">
			<input type="number"  placeholder="请输入手机" maxlength="11" class="phone2"  v-model="phone"/>
			<view class="code">
			...
			<view class="find_psw">
					<input type="password" v-model="newpsw" placeholder="请输入支付密码" class="newpassword1"/>
					<input type="password" v-model="newpsw1" placeholder="再次输入支付密码" class="newpassword2"/>
...
<script>
	...
	export default {
		...
		methods:{
			...
			loginNow: function() {
				this.$api.api.forgetPassword({phone:this.phone,TradePassword:this.newpsw,code:this.code,
				action:'RetrievePassword',type:2
				}).then((res)=>{
					console.log(res)
					if(res.data.status != true){
						uni.showToast({
